    ...
    size_t custom_claims_buffer_size)
{
    ...
    // Check SGX cpusvn
    value = _find_claim(claims, claims_size, OE_CLAIM_SGX_CPUSVN);
    OE_TEST(
        value != NULL &&
        memcmp(
    ...
    value = _find_claim(claims, claims_size, OE_CLAIM_PCESVN);
    OE_TEST(is_local || memcmp(value, &sgx_quote->pce_svn, sizeof(uint16_t)));

    value = _find_claim(claims, claims_size, OE_CLAIM_QEID);
    OE_TEST(
        is_local ||
    ...
    if (sgx_endorsements)
    {
        ...
        if (!is_local)
        {
            oe_parsed_tcb_info_t* parsed_tcb_info = NULL;
            ...
            value = _find_claim(claims, claims_size, OE_CLAIM_TCB_INFO_FMSPC);
            OE_TEST(value != NULL && memcmp(value, fmspc, sizeof(fmspc)) == 0);
        }
    }
